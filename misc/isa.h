...
typedef enum { I_HALT, I_NOP, I_RRMOVQ, I_IRMOVQ, I_RMMOVQ, I_MRMOVQ,
	       I_ALU, I_JMP, I_CALL, I_RET, I_PUSHQ, I_POPQ, I_LEAQ, I_VECADD, I_SHF} itype_t;

/* Different ALU operations */
typedef enum { A_ADD, A_SUB, A_AND, A_XOR, A_NONE } alu_t;

/* Different Shift operations */
typedef enum {S_HL, S_HR, S_AR, S_NONE} shf_t;
...
/* Get instruction byte from memory */
bool get_byte_val_I(mem_t m, word_t pos, byte_t *dest);

/* Get 8 instruction bytes from memory */
...
typedef struct memory_restore_struct {
    size_t num_values;
    word_t *positions;
    word_t *values;
} memory_restore_t;
...
typedef struct processor_state_struct {
    ...
    stat_t status;
    word_t icount;
    memory_restore_t *registers;
    memory_restore_t *memory;
} processor_state_t;